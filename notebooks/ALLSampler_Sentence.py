...
class VBSamplingSimulator(SamplingSimulator):
    ...
    def sort_dist_diff(self, distances:np.ndarray)->np.ndarray:
        if self.min_dist_diff:
            ...
            print("DISTANCE max_values each row", max_values, "size:", len(max_values))
            print("DISTANCE min_values each row", min_values, "size:", len(min_values))
            max_differences=max_values-min_values
            print("DISTANCE max_diff each row", max_differences, "size:", len(max_differences))
    
            mask= ~np.isnan(max_differences) #test element wise for NAN return boolean vec
            print("after masking max_diff size", len(max_differences[mask]) )
    ...
    def sample_next_round(self, sampled, remaining, randomly=True):
        ...
        logger.info(f'AFTER FAISS INDEXING distance shape: {distances.shape}, max to retrieve {max_retrieve} sentences')
        ...
        logger.info(f'AFTER FAISS INDEXING distance shape: {distances.shape}')
